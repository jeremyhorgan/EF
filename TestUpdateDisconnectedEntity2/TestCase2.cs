...
namespace TestUpdateDisconnectedEntity2
{
    ...
    /// Result:
    /// - The update took in the region of 1 second
    /// </remarks>
    internal class TestCase2
    {
        internal void Run()
        {
            ...
            var siteScenarioDatums = FetchSiteScenarioDatumForSiteScenario(siteScenario);

            siteScenario.SiteScenarioDatums = siteScenarioDatums;
            foreach (var siteScenarioDatum in siteScenarioDatums)
            {
                siteScenarioDatum.SiteScenario = siteScenario;
            }
        ...
        private static List<SiteScenario> FetchSiteScenarios()
        {
            ...
            {
                return context.SiteScenarios.ToList();
            }
        }

        private static List<SiteScenarioDatum> FetchSiteScenarioDatumForSiteScenario(SiteScenario siteScenario)
        {
            using (var context = new AppDbContext())
            {
                return context.SiteScenarioDatums
                    .Include(e => e.SiteEnrichments)
                    .Where(e => e.SiteScenarioId == siteScenario.Id)
                    .ToList();
            }
        }
